...
import type { ChatServerPayload, PollPayload } from "../types/payloads";
...
export class ServerInstance {
    ...
    postMessage(payload: ChatServerPayload): void {
        ...
        this.log(
            `Received message ID: ${msg.id}: total messages: ${this.messages.size}`
        );
    }
    ...
    deleteMessagesByUser(username: string): void {
        ...
        const messageIdsToDelete = Array.from(this.messages.entries())
            .filter(
                ([, msg]) => msg.action === "add" && msg.message?.username === username
            )
            .map(([id]) => id);
    ...
    private cleanOldMessages(): void {
        const now = Date.now();
        const keysToDelete = Array.from(this.messages.entries()).filter(
            ([, msg]) => {
        ...
        this.log(
            `Cleaned up ${keysToDelete.length} old messages. Remaining messages: ${this.messages.size}`
        );
    }
    ...
    private handlePoll(req: Request, res: Response): void {
        ...
        function doPoll(that: ServerInstance, uuid = ""): void {
            ...
            if (result.length > 0) {
                that.requestLogger(
                    req,
                    `Returning ${result.length} messages since token: ${token}`
                );
